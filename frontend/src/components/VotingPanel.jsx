...
function LiveVotesDisplay({ liveVotes }) {
            ...
            >
              ...
              <div className="flex items-center gap-2 text-sm">
                <span
                  ...
                  style={{ color: PLAYER_COLORS[vote.voter_name] || '#fff' }}
                >
                  {PLAYER_ICONS[vote.voter_name] || '🤖'}
                  <span className="font-medium">{vote.voter_name}</span>
                </span>
                <span className="text-gray-500">vota por</span>
                <span
                  className="flex items-center gap-1"
                  style={{ color: PLAYER_COLORS[vote.voted_for_name] || '#fff' }}
                >
                  {PLAYER_ICONS[vote.voted_for_name] || '🤖'}
  ...
  if (!humanPlayer) {
    ...
    return (
      <div className="bg-gray-800 rounded-xl p-6">
        ...
        <LiveVotesDisplay liveVotes={state.liveVotes} />
      </div>
    )
  }
  ...
  if (hasVoted) {
    return (
      <div className="bg-gray-800 rounded-xl p-6 text-center">
        <h3 className="text-lg font-medium mb-4">🗳️ Votación</h3>
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        {votablePlayers.map((player) => (
          ...
          >
            ...
            >
              {PLAYER_ICONS[player.display_name] || '🤖'}
            </div>
            <div>
